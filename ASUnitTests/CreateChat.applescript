...
on run
	tell application "AdiumY"
		...
		tell account (HandyAdiumScripts's defaultAccount)
			...
			if (count chat windows of application "AdiumY") is not c + 1 then
				close newChatWindow --restore
				error
			end
			if (count chats of application "AdiumY") is not c2 + 1 then
				close newChatWindow --restore
				error
			end
			...
			if (count chats of application "AdiumY") is not c2 + 2 then
				close newChatWindow
				error
			end
