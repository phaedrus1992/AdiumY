...
- (void)viewDidLoad
{
	...
	[tableView_accountList setAccessibilityTitle:AILocalizedString(@"Accounts", nil)];
...
- (void)configureAccountList
{
	...
	[cell setAccessibilityEnabled:YES];
				...
				row:(NSInteger)row
{
	...
	if ([identifier isEqualToString:@"enabled"]) {
		...
		[cell setAccessibilityTitle:[account.service longDescription]];
...
- (id<NSPasteboardWriting>)tableView:(NSTableView *)tv pasteboardWriterForRow:(NSInteger)row
{
	...
	NSMutableIndexSet *selection = [[tv selectedRowIndexes] mutableCopy];
	if (![selection containsIndex:(NSUInteger)row]) {
		[selection addIndex:(NSUInteger)row];
	}
	tempDragAccounts = [accountArray objectsAtIndexes:selection];
