...
+ (void)showSMPRequestForContact:(AIListContact *)contact question:(NSString *)question plugin:(AdiumOTREncryption *)plugin
{
	...
	if (question) {
		messageString = [NSString stringWithFormat:
			AILocalizedString(@"%@ wants to verify your identity with the question:\n\n%@\n\nDo you want to answer?", nil),
			contact.displayName, question];
	} else {
		messageString = [NSString stringWithFormat:
			AILocalizedString(@"%@ wants to verify your identity. Do you want to authenticate?", nil),
			contact.displayName];
	}
