...
- (void)completeLogin
{
	...
	@autoreleasepool {
		...
		[[NSNotificationCenter defaultCenter] postNotificationName:AIApplicationDidFinishLoadingNotification
															object:nil];
...
- (BOOL)application:(NSApplication *)theApplication openFile:(NSString *)filename
{
	...
	if (destination) {
		...
		NSAlert *xtraPrefsAlert = [[NSAlert alloc] init];
		xtraPrefsAlert.messageText = alertTitle;
		xtraPrefsAlert.informativeText = [NSString stringWithFormat:@"%@", alertMsg];
		[xtraPrefsAlert addButtonWithTitle:prefsButton];
		buttonPressed = [xtraPrefsAlert runModal];
		...
		installFailedAlert.messageText =
			AILocalizedString(@"Installation Failed", "Title of installation failed window");
		installFailedAlert.informativeText = [NSString stringWithFormat:@"%@", errorMessage];
		[installFailedAlert addButtonWithTitle:AILocalizedString(@"OK", nil)];
		[installFailedAlert runModal];
	}
...
- (NSString *)applicationSupportDirectory
{
	...
	if (!_preferencesFolderPath) {
		_preferencesFolderPath =
			[[[[NSBundle mainBundle] infoDictionary] objectForKey:PORTABLE_ADIUM_KEY] stringByExpandingTildeInPath];
		if (!_preferencesFolderPath)
	...
	if ([existingResourcePaths indexOfObject:targetPath] == NSNotFound) {
		...
		if (![defaultManager createDirectoryAtPath:targetPath
			...
			if (error) {
				...
				NSAlert *createFolderAlert = [[NSAlert alloc] init];
				createFolderAlert.alertStyle = NSAlertStyleCritical;
				createFolderAlert.messageText =
					[NSString stringWithFormat:AILocalizedString(@"Could not create the %@ folder.", nil), name];
				createFolderAlert.informativeText =
					AILocalizedString(@"Try running Repair Permissions from Disk Utility.", nil);
				[createFolderAlert addButtonWithTitle:AILocalizedString(@"OK", nil)];
...
- (void)noteEarliestLaunchedAdiumVersion
{
	if (![self earliestLaunchedAdiumVersion]) {
		/* Never noted before (first launch on this machine). Record the current version */
