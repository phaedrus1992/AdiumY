			...
			proposedChildIndex:(NSInteger)idx
{
	...
	} else if ([types containsObject:AINSPasteboardTypeFilenames] || [types containsObject:NSPasteboardTypeRTF] ||
			   [types containsObject:NSPasteboardTypeURL] || [types containsObject:NSPasteboardTypeString] ||
		 ...
		 childIndex:(NSInteger)idx
{
	...
	if ((availableType = [draggingPasteboard availableTypeFromArray:[NSArray arrayWithObject:@"AIListObject"]])) {
		...
		[[AIContactObserverManager sharedManager] endListObjectNotificationsDelay];
					...
					availableTypeFromArray:[NSArray arrayWithObjects:AINSPasteboardTypeFilenames,
		...
		if ([item isKindOfClass:[AIListContact class]]) {
			...
			NSBeep();
					...
					availableTypeFromArray:[NSArray arrayWithObjects:NSPasteboardTypeRTF, NSPasteboardTypeURL,
		...
		if ([item isKindOfClass:[AIListContact class]]) {
			...
			if ([availableType isEqualToString:NSPasteboardTypeRTF]) {
				...
				messageAttributedString =
					[NSAttributedString stringWithData:[[info draggingPasteboard] dataForType:NSPasteboardTypeRTF]];
				...
				messageAttributedString = [NSAttributedString
					stringWithString:[[info draggingPasteboard] stringForType:NSPasteboardTypeString]];
			}
	...
	for (AIListContact *listContact in [items arrayByAddingObject:inContact]) {
		...
		if (!listContact.canJoinMetaContacts) {
			...
			alert.informativeText = [NSString
				stringWithFormat:AILocalizedString(@"%@ is not able to be combined into a meta contact.", nil),
