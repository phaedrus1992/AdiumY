...
- (IBAction)promptForMultiples:(id)sender
{
    ...
    multipleImportPrompt.messageText = AILocalizedString(@"Have you used other chat clients?", "Title which introduces import assistants during setup");
    multipleImportPrompt.informativeText = AILocalizedString(@"Adium includes assistants to import your accounts, settings, and transcripts from other clients. Choose a client below to open its assistant, or press Continue to skip importing.", nil);
    [multipleImportPrompt addButtonWithTitle:AILocalizedStringFromTable(@"Continue", @"Buttons", nil)];
    [multipleImportPrompt addButtonWithTitle:AILocalizedString(@"Import from iChat", "iChat is the OS X instant messaging client which ships with OS X; the name probably should not be localized")];
    [multipleImportPrompt beginSheetModalForWindow:[self window]
                                  completionHandler:^(NSModalResponse returnCode) {
                                      [self multipleImportAlertDidEnd:multipleImportPrompt returnCode:returnCode contextInfo:nil];
                                  }];
}
