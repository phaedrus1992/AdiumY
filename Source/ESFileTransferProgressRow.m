...
- (id)initForFileTransfer:(ESFileTransfer *)inFileTransfer withOwner:(id)inOwner
{
	if ((self = [super init])) {
		...
		[[NSBundle mainBundle] loadNibNamed:@"ESFileTransferProgressView" owner:self topLevelObjects:NULL];
	}
...
{
	double updateTick = clock_gettime_nsec_np(CLOCK_UPTIME_RAW) / 1e9;
	...
	if (lastUpdateTick && ((updateTick - lastUpdateTick) < 0.2) && (status == In_Progress_FileTransfer) &&
		!forceUpdate) {
		return;
	}
	...
	if ((status == In_Progress_FileTransfer) && lastUpdateTick && lastBytesSent) {
		if (updateTick > lastUpdateTick) {
			if ([bytesSentQueue count] == 0) {
				...
				[updateTickQueue insertObject:[NSNumber numberWithDouble:lastUpdateTick] atIndex:0];
			...
			[updateTickQueue addObject:[NSNumber numberWithDouble:updateTick]];

			unsigned long long bytesDifference = bytesSent - [[bytesSentQueue objectAtIndex:0] unsignedLongLongValue];
			double ticksDifference = updateTick - [[updateTickQueue objectAtIndex:0] doubleValue];
			unsigned long long rate = bytesDifference / ticksDifference;
