...
@interface ESFileTransferController () <NSMenuItemValidation, NSToolbarItemValidation>
...
{
	ESFileTransfer *fileTransfer = [ESFileTransfer fileTransferWithContact:inContact forAccount:inAccount type:t];
...
- (BOOL)shouldOpenCompleteFileTransfer:(ESFileTransfer *)fileTransfer
{
	...
	if (autoOpenSafe && ([fileTransfer fileTransferType] == Incoming_FileTransfer)) {
		...
		if (!safeFileExtensions)
			safeFileExtensions = SAFE_FILE_EXTENSIONS_SET;
...
- (void)configureFileTransferProgressWindow
{
	...
	menuItem_showFileTransferProgress = [[NSMenuItem alloc] initWithTitle:AILocalizedString(@"File Transfers", nil)
															...
															keyEquivalent:@"l"];
	[menuItem_showFileTransferProgress
