...
@interface ESiTunesPlugin () <NSMenuItemValidation>
...
{
	NSString *itunesPath =
		[[[NSWorkspace sharedWorkspace] URLForApplicationWithBundleIdentifier:@"com.apple.iTunes"] path];
...
- (void)createiTunesToolbarItemWithPath:(NSString *)iTunesPath
{
	...
	if ([NSApp isOnLionOrNewer]) {
		...
		[button setImage:[[NSImage alloc] initWithData:imageData]];
	}
...
- (void)insertiTMSLink
{
	...
	if (![url length]) {
		...
		if (![self iTunesIsStopped] || ![self iTunesIsPaused]) {
			...
			if ([trackName length]) {
				[url appendFormat:@"n=%@", [trackName stringByAddingPercentEncodingWithAllowedCharacters:
			...
			if ([artist length] && [trackName length]) {
				...
				[url appendFormat:@"&an=%@", [artist stringByAddingPercentEncodingWithAllowedCharacters:
														 [NSCharacterSet URLQueryAllowedCharacterSet]]];
				...
				// stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
				[url appendFormat:@"an=%@", [trackName stringByAddingPercentEncodingWithAllowedCharacters:
														   [NSCharacterSet URLQueryAllowedCharacterSet]]];
			} else {
				artist = @"";
			}
...
- (void)searchMusicStoreWithSelection:(NSString *)selectedText
{
	...
	NSString *url = [NSString
		stringWithFormat:@"itms://phobos.apple.com/WebObjects/MZSearch.woa/wa/search?term=%@",
						 [selectedText
