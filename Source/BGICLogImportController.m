...
- (BOOL)createNewLogForPath:(NSString *)fullPath
{
	...
	@try {
		if ([[fullPath pathExtension] isEqual:@"ichat"]) {
			rawChat = [NSKeyedUnarchiver objectWithArchivedData:[NSData dataWithContentsOfFile:fullPath]];
		} else {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
			/* Older iChat .chatlog files were written with the non-keyed NSArchiver; NSKeyedUnarchiver
			 * cannot read them, so the deprecated NSUnarchiver fallback is required.
			 */
			rawChat = [NSUnarchiver unarchiveObjectWithFile:fullPath];
#pragma clang diagnostic pop
	...
	*/
	...
	filenameDateFormatter.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"];
	filenameDateFormatter.dateFormat = @"yyyy-MM-dd'T'HH.mm.ssZ";
		...
		stringByAppendingPathComponent:[NSString
										   stringWithFormat:@"%@ (%@).chatlog",
															...
															[filenameDateFormatter
																stringFromDate:[[[rawChat objectAtIndex:2]
																				   objectAtIndex:0] date]]]];
