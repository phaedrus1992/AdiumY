		...
		@autoreleasepool {
			@try {
				do {
					...
					// Continue to parse as long as we need more elements, we have data to read, and LMX doesn't think
					// we're done.
				} while ([foundMessages count] < linesLeftToFind && offset > 0 && result != LMXParsedCompletely);

			} @catch (id theException) {
				AILogWithSignature(@"Error \"%@\" while parsing %@; foundMessages at that point was %@, and the chunk "
								   @"to be parsed was %@",
