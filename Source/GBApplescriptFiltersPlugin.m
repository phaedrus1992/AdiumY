...
@interface GBApplescriptFiltersPlugin () <NSMenuItemValidation>
	...
	for (appendDict in scripts) {
		...
		if ((set = [appendDict objectForKey:@"Set"])) {
			...
			if (![set isEqualToString:lastSet]) {
				...
				item = [[NSMenuItem alloc] initWithTitle:set target:nil action:nil keyEquivalent:@""];
		...
		item = [[NSMenuItem alloc] initWithTitle:title target:self action:@selector(selectScript:) keyEquivalent:@""];
