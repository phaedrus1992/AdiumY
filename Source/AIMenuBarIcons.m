...
+ (NSImage *)previewMenuImageForIconPackAtPath:(NSString *)inPath
{
	...
	image = [[NSImage alloc]
		initWithSize:NSMakeSize((PREVIEW_MENU_IMAGE_SIZE + PREVIEW_MENU_IMAGE_MARGIN) * 2, PREVIEW_MENU_IMAGE_SIZE)];
...
- (NSImage *)alternateImageForImage:(NSImage *)inImage
{
	...
	[monochromeFilter setValue:[[CIImage alloc] initWithBitmapImageRep:srcImageRep] forKey:@"inputImage"];
	[monochromeFilter setValue:[NSNumber numberWithDouble:1.0] forKey:@"inputIntensity"];
	[monochromeFilter setValue:[[CIColor alloc] initWithColor:[NSColor whiteColor]] forKey:@"inputColor"];
