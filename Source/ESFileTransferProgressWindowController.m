...
- (void)_removeFileTransferRow:(ESFileTransferProgressRow *)progressRow
{
	...
	if ([fileTransfer isStopped]) {
		...
		if (shouldScrollToNewFileTransfer) {
			...
			NSPoint rowOrigin = [outlineView rectOfRow:row].origin;
			NSRect constrained = [clipView constrainBoundsRect:NSMakeRect(rowOrigin.x, rowOrigin.y, 0, 0)];
			[clipView scrollToPoint:constrained.origin];
