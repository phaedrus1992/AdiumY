...
@interface AIListContact (AIInfoInspectorPane_AddressBook)
/**
 * Provides an Address Book person record for this contact.
 * Implementation is a stub pending the Contacts Framework migration (issue 11).
 */
@property(readonly) ABPerson *addressBookPerson;
@end
...
- (void)updateUserIcon:(AIListObject *)inObject
{
	...
	/* NSImageScaleProportionallyDown will lock an animated GIF into a single frame.  We therefore use NSImageScaleNone
	 * if we are already at the right size or smaller than the right size; otherwise we scale proportionally to fit the
	 * frame.
	 */
...
@implementation AIListContact (AIInfoInspectorPane_AddressBook)
