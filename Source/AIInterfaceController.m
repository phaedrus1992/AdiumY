...
@interface AIInterfaceController () <NSMenuItemValidation>
...
- (void)controllerDidLoad
{
	...
	menuItem_toggleUserlist = [[NSMenuItem allocWithZone:NULL] initWithTitle:AILocalizedString(@"Toggle User List", nil)
																	  target:self
	...
	menuItem_toggleUserlistSide =
		[[NSMenuItem allocWithZone:NULL] initWithTitle:AILocalizedString(@"Toggle User List Side", nil)
												target:self
	...
	NSMenuItem *menuItem = [[NSMenuItem allocWithZone:NULL] initWithTitle:AILocalizedString(@"Toggle User List", nil)
																   target:self
																   action:@selector(toggleUserlist:)
															keyEquivalent:@""];
	...
	// Clear display
	menuItem_clearDisplay = [[NSMenuItem allocWithZone:NULL] initWithTitle:AILocalizedString(@"Clear Display", nil)
																	target:self
	...
	menuItem = [[NSMenuItem allocWithZone:NULL]
...
- (IBAction)reopenChat:(id)sender
{
	...
	if (!chat) {
		NSAlert *restoreChatFailedAlert = [[NSAlert alloc] init];
		restoreChatFailedAlert.messageText = AILocalizedString(@"Restoring chat failed", nil);
		restoreChatFailedAlert.informativeText =
			AILocalizedString(@"Restoring the last closed tab failed. Perhaps the account not exist anymore?", nil);
		[restoreChatFailedAlert addButtonWithTitle:AILocalizedString(@"OK", nil)];
		[restoreChatFailedAlert runModal];
		return;
	}
...
- (void)updateActiveWindowMenuItem
{
	...
	for (item in windowMenuArray) {
		if ([item representedObject])
			[item setState:([item representedObject] == activeChat ? NSControlStateValueOn : NSControlStateValueOff)];
	}
}
...
- (void)buildWindowMenu
{
	...
	for (NSDictionary *containerDict in [interfacePlugin openContainersAndChats]) {
		...
		if (contentArray.count > 1) {
			item = [[NSMenuItem allocWithZone:NULL]
		...
		for (AIChat *chat in [contentArray objectEnumerator]) {
			...
			item = [[NSMenuItem allocWithZone:NULL] initWithTitle:chat.displayName
														   target:self
		...
		if (object == tooltipListObject) { // If we already have this tooltip open
			...
			if (tooltipImage) {
				...
				tabArray = [[NSArray alloc]
					initWithObjects:[[NSTextTab alloc] initWithType:NSRightTabStopType location:maxLabelWidth],
...
- (IBAction)toggleFontPanel:(id)sender
{
	if ([NSFontPanel sharedFontPanelExists] && [[NSFontPanel sharedFontPanel] isVisible]) {
		...
		if (!fontPanelAccessoryView) {
			[[NSBundle mainBundle] loadNibNamed:@"FontPanelAccessoryView" owner:self topLevelObjects:NULL];
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
	...
	if (menuItem == menuItem_bold || menuItem == menuItem_italic) {
		...
		NSArray *nonImageTypes =
			[NSArray arrayWithObjects:NSPasteboardTypeString, NSPasteboardTypeRTF, NSPasteboardTypeURL,
									  AINSPasteboardTypeFilenames, NSFilesPromisePboardType, NSPasteboardTypeRTFD, nil];
...
- (NSMenu *)menuForWindowLevelsNotifyingTarget:(id)target
{
	...
	menuItem = [[NSMenuItem allocWithZone:NULL] initWithTitle:AILocalizedString(@"Above other windows", nil)
													   target:target
	...
	menuItem = [[NSMenuItem allocWithZone:NULL] initWithTitle:AILocalizedString(@"Normally", nil)
													   target:target
													   action:@selector(selectedWindowLevel:)
												keyEquivalent:@""];
	[menuItem setEnabled:YES];
	...
	menuItem = [[NSMenuItem allocWithZone:NULL] initWithTitle:AILocalizedString(@"Below other windows", nil)
													   target:target
													   action:@selector(selectedWindowLevel:)
												keyEquivalent:@""];
	[menuItem setEnabled:YES];
