...
#define LOG_SEARCH_STATUS_INTERVAL (20.0 / 60.0) // ~1/3 s to wait before refreshing search status
...
@interface AILogViewerWindowController () <NSToolbarItemValidation, NSControlTextEditingDelegate>
...
#define HORIZONTAL_RULE_LENGTH 18
						...
						horizontalRule = [[NSString alloc] initWithCharacters:separatorUTF16
																	   length:HORIZONTAL_RULE_LENGTH];
					}
				...
				if ([[theLog relativePath] hasSuffix:@".AdiumHTMLLog"] || [[theLog relativePath] hasSuffix:@".html"] ||
					...
					// HTML log
					NSURL *logURL =
						[NSURL fileURLWithPath:[logBasePath stringByAppendingPathComponent:[theLog relativePath]]];
					NSString *logFileText = [NSString stringWithContentsOfURL:logURL
					...
					if ([[NSFileManager defaultManager] fileExistsAtPath:logFullPath isDirectory:&isDir]) {
						...
						if (isDir)
							logFullPath =
								[logFullPath stringByAppendingPathComponent:[[[logFullPath lastPathComponent]
					...
					if (logFileText) {
						...
						if (showEmoticons) {
							attributedLogFileText =
								[adium.contentController filterAttributedString:attributedLogFileText
																usingFilterType:AIFilterMessageDisplay
																	  direction:AIFilterOutgoing
																		context:nil];
						}
				...
				while (![scanner isAtEnd]) {
					@autoreleasepool {
						...
						if (![scanner isAtEnd] && [scanner scanUpToString:@"\"" intoString:&quotedString]) {
							...
							/* If a string within quotes is found, remove the words from the quoted string and add the
							 * full string to what we'll be highlighting.
							 *
							 * We'll use indexOfObject: and removeObjectAtIndex: so we only remove _one_ instance.
							 * Otherwise, this string: "killer attack ninja kittens" OR ninja wouldn't highlight the
							 * word ninja by itself.
							 */
...
- (void)_logContentFilter:(NSString *)searchString searchID:(NSInteger)searchID onSearchIndex:(SKIndexRef)logSearchIndex
{
	...
	thisSearch = SKSearchCreate(logSearchIndex, (__bridge CFStringRef)wildcardedSearchString, kSKSearchOptionDefault);
...
- (void)_logFilter:(NSString *)searchString searchID:(NSInteger)searchID mode:(LogSearchMode)mode
{
	double lastUpdate = CACurrentMediaTime();

	NSDate *searchStringDate = nil;

	if ((mode == LOG_SEARCH_DATE) && (searchString != nil)) {
		NSDataDetector *dateDetector = [NSDataDetector dataDetectorWithTypes:NSTextCheckingTypeDate error:NULL];
		searchStringDate = [[dateDetector firstMatchInString:searchString
													 options:0
													   range:NSMakeRange(0, [searchString length])] date];
	}
					...
					for (AIChatLog *theLog in [toGroup logEnumerator]) {
						...
						if ((mode != LOG_SEARCH_DATE) || (!searchString) ||
							...
							if ([self chatLogMatchesDateFilter:theLog]) {
								...
								// Update our status
								if (lastUpdate == 0.0 ||
									CACurrentMediaTime() > lastUpdate + LOG_SEARCH_STATUS_INTERVAL) {
	...
	if ([selectedLogs count] > 0) {
		...
		[alert beginSheetModalForWindow:[self window]
					  completionHandler:^(NSModalResponse returnCode) {
						  [self deleteLogsWithReturnCode:returnCode selectedLogs:selectedLogs];
					  }];
	}
}
	...
	if ([item isKindOfClass:[AIMetaContact class]] &&
		...
		if ([[outlineView arrayOfSelectedItems] containsObjectIdenticalTo:item] &&
			...
			if (!adiumIconHighlighted) {
				adiumIconHighlighted = [NSImage imageNamed:@"adiumHighlight" forClass:[self class]];
			}
...
- (void)installToolbar
{
	...
	[[NSBundle mainBundle] loadNibNamed:[self dateItemNibName] owner:self topLevelObjects:&topLevelObjects];
...
{
	NSMenuItem *menuItem =
		[[NSMenuItem alloc] initWithTitle:[dateTypeTitleDict objectForKey:[NSNumber numberWithInteger:dateType]]
...
{
	NSInteger todayDayOfWeek = [[[NSCalendar currentCalendar] components:NSCalendarUnitWeekday fromDate:today] weekday];
...
- (void)restoreDeletedLogs:(NSArray *)deletedLogs
{
	...
	NSString *trashPath =
		[NSSearchPathForDirectoriesInDomains(NSTrashDirectory, NSUserDomainMask, YES) objectAtIndex:0];
...
- (void)deleteLogsWithReturnCode:(NSModalResponse)returnCode selectedLogs:(NSArray *)selectedLogs
{
...
- (void)deleteLogs:(NSArray *)selectedLogs
{
	if ([selectedLogs count] > 1) {
		...
		[self deleteLogsWithReturnCode:NSAlertFirstButtonReturn selectedLogs:selectedLogs];
	}
}
...
- (void)deleteSelectedContactsFromSourceListWithReturnCode:(NSModalResponse)returnCode
										  selectedToGroups:(NSArray *)allSelectedToGroups
{
	if (returnCode == NSAlertFirstButtonReturn) {
		AILogToGroup *logToGroup;
		NSMutableSet *logPaths = [NSMutableSet set];
...
- (void)deleteSelectedContactsFromSourceList
{
	...
	if (totalLogCount > 1) {
		...
		[alert beginSheetModalForWindow:[self window]
					  completionHandler:^(NSModalResponse returnCode) {
						  [self deleteSelectedContactsFromSourceListWithReturnCode:returnCode
																  selectedToGroups:allSelectedToGroups];
					  }];
	} else {
		[self deleteSelectedContactsFromSourceListWithReturnCode:NSAlertFirstButtonReturn
												selectedToGroups:allSelectedToGroups];
	}
}
