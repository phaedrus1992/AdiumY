...
- (void)testEmoticonCreationWithRandomTextEquivalents
{
	PBTCheckDefault({
		...
		AIEmoticon *emoticon = [AIEmoticon emoticonWithIconPath:path equivalents:equivalents name:name pack:pack];
		...
		if (emoticon) {
			...
			if ([equivalents count] > 0) {
				STAssertEqualObjects([emoticon textEquivalents], equivalents, @"Emoticon text equivalents mismatch");
			}
		}
	});
}
...
{
	AIEmoticon *emoticon = [AIEmoticon emoticonWithIconPath:@"/tmp/test.png" equivalents:@[] name:@"test" pack:nil];
...
- (void)testEmoticonEnabledState
{
	PBTCheckDefault({
		...
		STAssertEquals([emoticon isEnabled], !initialState, @"Emoticon enabled state should toggle");
		[emoticon setEnabled:initialState];
		STAssertEquals([emoticon isEnabled], initialState, @"Emoticon enabled state should toggle back");
	});
}
...
- (void)testIsAppropriateForServiceClassWithRandomInput
{
	PBTCheckDefault({
		...
		STAssertNoThrowSpecific([emoticon isAppropriateForServiceClass:serviceClass], NSException,
